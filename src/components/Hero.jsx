...
import {heroIcons, socialMediaData} from '@/assets'
        ...
        <div>
            <motion.div initial={{opacity:0, y:-100}} animate={{opacity:1, y:0}} transition={{delay:.5}} className='flex flex-col items-center justify-center gap-y-3 font-light capitalize'>
                ...
                style={{rotateX: mouseMove ? rotateX : 0,
                    ...
                    initial={{scale:0}} animate={{opacity: buttonHover ? 0 : 1, scale: buttonHover ? 2 : 0, y: buttonHover? -40: 0}}
                    transition={{opacity:{delay:0.4}}}>Hi</motion.span>
                </motion.div>
                <h1 className='text-center text-3xl font-bold tracking-wider text-gray-500 sm:text-2xl dark:text-white transition-colors'>My name is Eric Zhu &</h1>
            ...
            {socialMediaData.map((item,i)=> (
               <a href={item.link} target="_blank" rel="noopener noreferrer" key={i} className='rounded-lg hover:bg-red-400 hover:text-white transition-colors'>
                {item.icon}
                </a>
            ))}
            </motion.div>
